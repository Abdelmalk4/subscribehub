...
export function UpgradePlanDialog({
  ...
  const getPriceLabel = () => {
    if (billingCycle === "yearly") {
      return `$${getAdjustedPrice().toFixed(2)}/year`;
    }
    ...
    try {
      ...
      const { data: invoice, error } = await supabase
        ...
        .insert({
          ...
          notes: billingCycle === "yearly" ? "Yearly billing (20% discount applied)" : "Monthly billing",
        })
        .select("id")
        .single();
          ...
          <div className="space-y-4">
            <div className="bg-muted/30 rounded-lg p-4 space-y-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-sm capitalize">{billingCycle}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground text-sm">Price</span>
                <div className="text-right">
                  <span className="font-medium text-sm">{getPriceLabel()}</span>
                  {billingCycle === "yearly" && (
                    <p className="text-xs text-green-600">20% discount applied</p>
                  )}
                </div>
              </div>
              <div className="flex justify-between">
