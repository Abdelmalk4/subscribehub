...
export default function SocialProofSection() {
  ...
  return (
    <section ref={sectionRef} className="py-12 px-4 sm:px-6 lg:px-8 fade-in-up">
      <div className="max-w-5xl mx-auto text-center">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10">
          {logos.map((logo) => (
            <div
              key={logo.name}
              className="logo-grayscale text-lg font-bold text-muted-foreground/70 hover:text-foreground transition-all cursor-default"
            >
              {logo.text}
            </div>
          ))}
